...
import { configuracionFrontendController } from './configuracionFrontendController.js';

export const paletaColorController = {

    async inicializar() {
        // 1. Inyectar la base (esto crea los contenedores #paletasContainer y #settingsContainer)
        const response = await fetch('views/configuracion/index.html');
        const htmlBase = await response.text();
        document.getElementById('content-area').innerHTML = htmlBase;

        // 2. Cargar datos de Paletas
        const paletas = await paletaColorModel.obtenerTodas();
        if (paletas) {
            paletaColorView.renderizarLista(paletas);
        }

        // 3. ¡ESTO ES LO QUE FALTA! Cargar los parámetros generales
        // Llamamos al otro controlador para que rellene su parte del HTML
        await configuracionFrontendController.inicializarIndex();
    },
